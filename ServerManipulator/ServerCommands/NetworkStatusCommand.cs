namespace WebApi.Services.ServerCommands
{
    ...
    public static class NetworkStatusCommand
    {
        public static int? GetNetworkIoSpeed(this TargetMachine client)
        {
            ...
            var duration = 0.1;
            var ethName = "eno1";
            using var cmd = client.RunCommand(@$"read s1 < /sys/class/net/{ethName}/statistics/rx_bytes; sleep {duration}; read s2 < /sys/class/net/{ethName}/statistics/rx_bytes; echo $(($s2-$s1))");
