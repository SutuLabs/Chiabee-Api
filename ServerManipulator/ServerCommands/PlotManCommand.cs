namespace WebApi.Services.ServerCommands
{
    ...
    public static class PlotManCommand
    {
        public static bool StopPlot(this TargetMachine client, string plotId)
        {
            ...
            using var cmd = client.ExecuteCommand($@". ~/chia-blockchain/activate && yes | plotman kill {plotId}");
        ...
        {
            if (!client.EnsureConnected()) return default;
            using var cmd = client.ExecuteCommand($@"cat ~/.config/plotman/plotman.yaml");
            var yaml = cmd.Result;
        ...
        public static bool SetPlotManConfiguration(this TargetMachine m, PlotManConfiguration config)
        {
            ...
            {
                using var cresult = m.ExecuteCommand($"sed -i 's/{leading}: {placeholder}/{leading}: {value}/g' ~/.config/plotman/plotman.yaml");
        ...
        public static bool CleanLegacyTemporaryFiles(this TargetMachine client)
        {
            ...
            using var cmd = client.ExecuteCommand($@"ls /data/tmp/ |sort |grep -o '.*\.plot' |sort -u");
            ...
            {
                using var cleanCmd = client.ExecuteCommand($@"rm /data/tmp/*{id}*");
